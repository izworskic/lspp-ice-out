...
async function fetchShard(base,key,label){
  const r=await fetch(`${base}/${key}.json`,{headers:{Accept:'application/json'}});
  if(r.status===404)return [];
  if(!r.ok)throw new Error(`${label} lake shard ${r.status}`);
  const rows=await r.json();
  return Array.isArray(rows)?rows:[];
}
async function searchUS(parsed,key){
  if(parsed.country==='CA')return [];
  let rows=(await fetchShard(US_INDEX_BASE,key,'US')).filter(x=>Array.isArray(x)&&x[4]==='US');
  if(parsed.regionCode&&parsed.country==='US')rows=rows.filter(x=>x[2]===parsed.regionCode);
  return rows;
}
async function searchCanada(parsed,key){
  if(parsed.country==='US')return [];
  let rows=(await fetchShard(CA_INDEX_BASE,key,'Canada')).filter(x=>Array.isArray(x)&&x[4]==='CA');
  if(parsed.regionCode&&parsed.country==='CA')rows=rows.filter(x=>x[2]===parsed.regionCode);
  return rows;
}
...
module.exports=async function handler(req,res){
  ...
  const settled=await Promise.allSettled([searchUS(parsed,key),searchCanada(parsed,key)]);
  ...
  return res.end(JSON.stringify({query:q,name_query:parsed.nameQuery,region:parsed.regionCode,country:parsed.country,shard:key,count:rows.length,rows,sources:{US:'USGS GNIS static index',CA:'NRCan CGNDB static index'}}));
};
