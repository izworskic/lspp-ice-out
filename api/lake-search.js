...
const INDEX_BASE=`https://cdn.jsdelivr.net/gh/izworskic/lspp-ice-out@${DATA_COMMIT}/north-america/data/lake-index`;
...
module.exports=async function handler(req,res){
  ...
  try{
    const r=await fetch(`${INDEX_BASE}/${key}.json`,{headers:{Accept:'application/json'}});
    if(!r.ok)throw new Error(`lake shard ${r.status}`);
    let rows=await r.json();
    rows=(Array.isArray(rows)?rows:[]).filter(x=>Array.isArray(x)&&x[4]==='US');
    if(parsed.regionCode)rows=rows.filter(x=>x[2]===parsed.regionCode);
    rows=rows.map(x=>({x,s:score(parsed.nameQuery,x)})).filter(o=>o.s>0).sort((a,b)=>b.s-a.s||String(a.x[1]).localeCompare(String(b.x[1]))||String(a.x[3]).localeCompare(String(b.x[3]))).slice(0,40).map(o=>o.x);
    res.setHeader('Content-Type','application/json; charset=utf-8');
    res.setHeader('Cache-Control','public, s-maxage=86400, stale-while-revalidate=604800');
    return res.end(JSON.stringify({query:q,name_query:parsed.nameQuery,state:parsed.regionCode,shard:key,count:rows.length,rows}));
  }catch(e){res.statusCode=502;res.setHeader('Cache-Control','no-store');return res.end(JSON.stringify({error:'lake_search_unavailable',detail:e.message}));}
};
