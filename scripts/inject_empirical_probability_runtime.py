...
if "lake.history.doys.reduce((sum,d)=>sum+1/(1+Math.exp(-(targetDoy+shift-d)/3))" in s and "p25Doy:Number(h.p25_doy)" in s:
    print('Empirical probability runtime already applied')
    raise SystemExit(0)
